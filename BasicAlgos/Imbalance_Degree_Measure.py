...
def Optimize(X_train, y_train):
    ...
    def birch_id(k):
        return Birch_ID(k=int(k), X_train=X_train)
    def gmm_id(k):
        return GMM_ID(k=int(k), X_train=X_train)
    def bis_kmeans_id(k):
        return Bis_KMeans_ID(k=int(k), X_train=X_train)
    ...
    for i in range(len(Cluster_Methods)):
        ...
        df_res.loc[i,'Cluster_Method'] = Cluster_Methods[i]+'_ID'
