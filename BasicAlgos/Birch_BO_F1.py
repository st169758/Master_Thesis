...
def Optimize_birch(X_train, y_train):
    #Apply Bayesian Optimization to K-Means
    def birch_f1(k):
        return Birch_F1(k=int(k), X_train=X_train)
    n = np.unique(y_train)
...
k_opt = Optimize_birch(X_train, y_train)
print()
...
df_res.loc[1,'DCM'] = "F1-Measure"
df_res.loc[1,'Clustering_Method'] = "Birch"
df_res.loc[1,'Opt-k'] = k_opt
df_res.loc[1,'Accuracy'] = accuracy_score(df_test['y_true'],df_test['y_pred'])
df_res.loc[1,'F1-score'] = f1_score(df_test['y_true'],df_test['y_pred'],average='weighted')
df_res.to_csv('./Results.csv', mode='a', header=False, index=False)