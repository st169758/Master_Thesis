...
def Optimize(X_train, y_train):
    ...
    def birch_f3(k):
        return Birch_F3(k=int(k), X_train=X_train)
    def gmm_f3(k):
        return GMM_F3(k=int(k), X_train=X_train)
    def bis_kmeans_f3(k):
        return Bis_KMeans_F3(k=int(k), X_train=X_train)
    ...
    for i in range(len(Cluster_Methods)):
        ...
        df_res.loc[i,'Cluster_Method'] = Cluster_Methods[i]+'_F3'
