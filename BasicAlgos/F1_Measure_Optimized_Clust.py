...
def Optimize(X_train, y_train):
    ...
    def opt(k,Cluster_Method):
        i = Cluster_Method
        if i == 0:
            func = KMeans_F1(k=int(k), X_train=X_train)
        elif i == 1:
            func = Birch_F1(k=int(k), X_train=X_train)
        elif i == 2:
            func = GMM_F1(k=int(k), X_train=X_train)
        elif i == 3:
            func = Bis_KMeans_F1(X_train=X_train, k=int(k))
        return func
    ...
    df_res.loc[0,'Cluster_Method'] = str(optimizer.max['params']['Cluster_Method']) + '_F1'
...
print(df_res)
df_res.to_csv('./Results1.csv', index=False, header=True)
