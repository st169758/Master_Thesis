...
def Optimize_gmm(X_train, y_train):
    #Apply Bayesian Optimization to K-Means
    def gmm_f1(k):
        return GMM_F1(k=int(k), X_train=X_train)
    n = np.unique(y_train)
...
k_opt = Optimize_gmm(X_train, y_train)
print()
...
df_res.loc[2,'DCM'] = "F1-Measure"
df_res.loc[2,'Clustering_Method'] = "GMM"
df_res.loc[2,'Opt-k'] = k_opt
df_res.loc[2,'Accuracy'] = accuracy_score(df_test['y_true'],df_test['y_pred'])
df_res.loc[2,'F1-score'] = f1_score(df_test['y_true'],df_test['y_pred'],average='weighted')
