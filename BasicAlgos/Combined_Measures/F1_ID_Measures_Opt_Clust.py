...
from Imbalance_Degree import ImbalanceDegree, class_dist
...
def KMeans_F1_F2(k, X_train):
    ...
    for clust_id in range(k):
        ...
        ID = ID.value
        df_clust.append(F1+ID)

    return np.mean(df_clust)


def Birch_F1_F2(k, X_train):
...
def Bis_KMeans_F1_F2(X_train, k):
...
def Optimize(X_train, y_train):
    ...
    def opt(k,Cluster_Method):
        i = Cluster_Method
        if i == 0:
            func = KMeans_F1_F2(k=int(k), X_train=X_train)
        elif i == 1:
            func = Birch_F1_F2(k=int(k), X_train=X_train)
        elif i == 2:
            func = GMM_F1_F2(k=int(k), X_train=X_train)
        elif i == 3:
            func = Bis_KMeans_F1_F2(X_train=X_train, k=int(k))
        return func
    ...
    df_res.loc[0,'Cluster_Method'] = str(optimizer.max['params']['Cluster_Method']) + '_F1_ID'
