...
from functions import ft_F1,ft_F2, ft_F3
...
def KMeans_F1_F2_F3(k, X_train):
    ...
    for clust_id in range(k):
        ...
        df_clust.append(F1+F2+F3)

    return np.mean(df_clust)


def Birch_F1_F2_F3(k, X_train):
...
def Bis_KMeans_F1_F2_F3(X_train, k):
...
def Optimize(X_train, y_train):
    ...
    def opt(k,Cluster_Method):
        i = Cluster_Method
        if i == 0:
            func = KMeans_F1_F2_F3(k=int(k), X_train=X_train)
        elif i == 1:
            func = Birch_F1_F2_F3(k=int(k), X_train=X_train)
        elif i == 2:
            func = GMM_F1_F2_F3(k=int(k), X_train=X_train)
        elif i == 3:
            func = Bis_KMeans_F1_F2_F3(X_train=X_train, k=int(k))
        return func
    ...
    df_res.loc[0,'Cluster_Method'] = str(optimizer.max['params']['Cluster_Method']) + '_F1_F2_F3'
