...
def Optimize(X_train, y_train):
    ...
    def birch_f2(k):
        return Birch_F2(k=int(k), X_train=X_train)
    def gmm_f2(k):
        return GMM_F2(k=int(k), X_train=X_train)
    def bis_kmeans_f2(k):
        return Bis_KMeans_F2(k=int(k), X_train=X_train)
    ...
    for i in range(len(Cluster_Methods)):
        ...
        df_res.loc[i,'Cluster_Method'] = Cluster_Methods[i]+'_F2'
