...
from functions import ft_T4, precompute_pca_tx
...
def Optimize(X_train, y_train):
    ...
    def birch_t4(k):
        return Birch_T4(k=int(k), X_train=X_train)
    def gmm_t4(k):
        return GMM_T4(k=int(k), X_train=X_train)
    def bis_kmeans_t4(k):
        return Bis_KMeans_T4(k=int(k), X_train=X_train)
    ...
    for i in range(len(Cluster_Methods)):
        ...
        df_res.loc[i,'Cluster_Method'] = Cluster_Methods[i]+'_T4'
